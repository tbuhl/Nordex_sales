...
APP_TITLE = "Nordex Sales Intelligence Dashboard"
DATA_DIR = Path("data")
DATA_CACHE_DIR = Path("data_cache")
PARSED_DATA_FILE = DATA_CACHE_DIR / "nordex_parsed_data.pkl"
STOCK_MONTHLY_FILE = DATA_DIR / "nordex_stock_monthly.json"
MARKET_MONTHLY_FILE = DATA_DIR / "market_prices_monthly.json"

NEWS_FEEDS = [
    {
        "source": "Windpower Monthly - Nordex",
        "url": "https://www.windpowermonthly.com/rss/nordex",
        "focus": "industry",
    },
    {
        "source": "OffshoreWIND.biz - Nordex",
        "url": "https://www.offshorewind.biz/?s=nordex&feed=rss2",
        "focus": "industry",
    },
    {
        "source": "Google News (Reuters + Nordex)",
        "url": "https://news.google.com/rss/search?q=site%3Areuters.com%20Nordex%20wind&hl=en-US&gl=US&ceid=US:en",
        "focus": "financial",
    },
    {
        "source": "Google News (Bloomberg + Nordex)",
        "url": "https://news.google.com/rss/search?q=site%3Abloomberg.com%20Nordex%20wind&hl=en-US&gl=US&ceid=US:en",
        "focus": "financial",
    },
    {
        "source": "Google News (Recharge + Nordex)",
        "url": "https://news.google.com/rss/search?q=site%3Arechargenews.com%20Nordex%20wind&hl=en-US&gl=US&ceid=US:en",
        "focus": "industry",
    },
    {
        "source": "Google News (Power Technology + Nordex)",
        "url": "https://news.google.com/rss/search?q=site%3Apower-technology.com%20Nordex%20wind&hl=en-US&gl=US&ceid=US:en",
        "focus": "technical",
    },
]

NORDEX_KEYWORDS = ("nordex", "ndx1", "n149", "n163", "n175")
NORDEX_NEWS_BLACKLIST = (
    "your career at nordex",
    "this is nordex",
    "onshore wind turbines - nordex",
    "offshore wind turbines - nordex",
    "from 2005-2008 - nordex",
...
NORDEX_TITLE_RE = re.compile(r"\b(?:nordex|ndx1|n149|n163|n175)\b", re.IGNORECASE)
PATENTSCOPE_SEARCH_URL = (
    "https://patentscope.wipo.int/search/en/result.jsf"
    "?query=PA%3A%22Nordex%22&sortOption=-DP"
)
...
PLATFORM_SLOT_SPECS = [
    {
        "slot": 1,
        "qty": ["#", "No_WTG_1", "NumberofTurbines"],
        "platform": ["platform", "TurbineType"],
        "rotor": ["Rotor", "Rotor_1"],
        "mw": ["MW", "MW_1"],
    },
    {
...
def normalize_metric_name(value: Any) -> str | None:
    ...
    text = text.replace("renevue", "revenue").replace("capcacity", "capacity").replace("employyes", "employees")
    text = text.replace("pr emp", "per employee")
...
<div class="sidebar-footer-wrap">
  ...
  Unofficial dashboard - not affiliated with or endorsed by Nordex. Data compiled manually from public Nordex reports/announcements; verify against original sources. Provided "as is" without warranty.
  </div>
</div>
        """,
        unsafe_allow_html=True,
...
def normalize_service_scheme(value: Any) -> str:
    ...
    if norm in SVC_UNKNOWN_VALUES:
        return "Unknown"
    if compact in {"premium", "premiumservice"}:
        return "Premium"
    if compact in {"basic", "baseservice"}:
        return "Basic"
    if compact in {"fullservice", "full"}:
        return "Full Service"
    if "5000" in compact:
        return "AOM5000"
    if "4000" in compact:
...
def normalize_platform(value: Any) -> str:
    ...
    # Normalize malformed patterns like N163/N5.X.
    text = re.sub(r"^([A-Z]{1,3}\d{2,3})/N([0-9]+(?:\.[0-9]+)?)\.?X$", r"\1/\2.X", text)

    # Handle multi-rating notation like N117/3000/3600 by grouping to family.
    multi_rating = re.fullmatch(r"([A-Z]{1,3}\d{2,3})/(\d{3,4})(?:/(\d{3,4}))+?", text)
    if multi_rating:
        rotor_label = multi_rating.group(1)
        first_rating = multi_rating.group(2)
        power_digit = first_rating[0]
        return f"{rotor_label}/{power_digit}.X"

    # Keep rotor-only labels as-is (e.g. N133).
    m = re.fullmatch(r"([A-Z]{1,3}\d{2,3})", text)
    if m:
        return m.group(1)

    # Normalize platform families, e.g. N131/3000 -> N131/3.X.
    m = re.fullmatch(r"([A-Z]{1,3}\d{2,3})[-/]([0-9]{4})", text)
    if m:
        rotor_label = m.group(1)
        power_digit = m.group(2)[0]
        return f"{rotor_label}/{power_digit}.X"

    m = re.fullmatch(r"([A-Z]{1,3}\d{2,3})[-/]N?([0-9]+(?:\.[0-9]+)?)\.?X", text)
    if m:
        rotor_label = m.group(1)
        power = float(m.group(2))
        return f"{rotor_label}/{int(math.floor(power))}.X"

    m = re.fullmatch(r"([A-Z]{1,3}\d{2,3})[-/]([0-9]+(?:\.[0-9]+)?)(MW|KW)", text)
    if m:
        rotor_label = m.group(1)
        power = float(m.group(2))
        if m.group(3) == "KW":
    ...
    m = re.fullmatch(r"([A-Z]{1,3}\d{2,3})[-/]([0-9]+(?:\.[0-9]+)?)", text)
    if m:
        rotor_label = m.group(1)
        power = float(m.group(2))
        if power >= 1000:
...
def parse_platform_specs(platform: str) -> tuple[float, float]:
    ...
    rotor_prefix = r"(?:[A-Z]{1,3})(\d{2,3})"
    # Canonical grouped family like N163/6.X
    grouped = re.search(rf"{rotor_prefix}[-/]([0-9]+(?:\.[0-9]+)?)\.?X", p)
    if grouped:
        rotor = float(grouped.group(1))
        power = float(grouped.group(2))
        return rotor, power

    full = re.search(rf"{rotor_prefix}[-/ ]([0-9]+(?:\.[0-9]+)?)(?:X|MW|KW)", p)
    if full:
        rotor = float(full.group(1))
        power = float(full.group(2))
        return rotor, power

    full_alt = re.search(rf"{rotor_prefix}[-/ ]([0-9]+(?:\.[0-9]+)?)(MW|KW)", p)
    if full_alt:
        rotor = float(full_alt.group(1))
        power = float(full_alt.group(2))
        if full_alt.group(3) == "KW":
            power = power / 1000.0
        return rotor, power
    ...
    rotor_only = re.search(rf"{rotor_prefix}$", p)
    if rotor_only:
        return float(rotor_only.group(1)), np.nan
...
def parse_economy_sheet(workbook: Path) -> pd.DataFrame:
    ...
    for cand in ["economy", "Economy", "Nordex Economy", "Nordex economy"]:
        if cand in xls.sheet_names:
            sheet_name = cand
            break
    if sheet_name is None:
        for s in xls.sheet_names:
            if "econom" in s.lower():
                sheet_name = s
                break
    if sheet_name is None:
    ...
    for i in range(nrows):
        ...
        first_year_col = min(j for j, _ in year_cols)
        year_map = {j: y for j, y in year_cols}
        blank_run = 0

        for k in range(i + 1, min(i + 60, nrows)):
            vals = raw.iloc[k].tolist()
            left = [clean_text(vals[c]) for c in range(first_year_col)]
            left_nonempty = [t for t in left if t is not None]

            metric = left_nonempty[-1] if left_nonempty else None
            if metric is None:
                blank_run += 1
                if blank_run >= 5:
                    break
            ...
            if metric_norm is None:
                continue
            metric_low = metric_norm.lower().strip()
            if metric_low in {"country", "q1", "q2", "q3", "q4", "in %"}:
                continue
            if re.fullmatch(r"\d{4}", metric_low):
                continue

            group_label = left_nonempty[-2] if len(left_nonempty) >= 2 else None
            if group_label:
                group_norm = normalize_metric_name(group_label)
            else:
                group_norm = None

            metric_name = metric_norm
            if group_norm and group_norm.lower() not in {metric_low, "country"}:
                if metric_low not in {"total"}:
                    metric_name = f"{group_norm} - {metric_norm}"

            values: dict[int, float] = {}
            for col_idx, year in year_map.items():
                value = to_float(vals[col_idx])
...
def parse_oi_sheets(workbook: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    oi_sheets = [(s, parse_oi_sheet_year(s)) for s in xl.sheet_names]
    ...
    for sheet, year in oi_sheets:
        ...
        for row_idx, row in block.iterrows():
            ...
            order_date = parse_mixed_date(first_text(row, ["date"]))
            size_mw = first_float(row, ["size [MW]", "MWsize", "size [mw]"])
            service_time_years = first_float(row, ["Service time  [years]", "Service time [years]", "service_len"])

            delivery_q = first_float(row, ["delivery q", "del q"])
            delivery_year = first_float(row, ["delivery y", "del year"])
            delivery_days = first_float(row, ["Delivery days", "delivery days"])
            ...
            country = normalize_country(first_text(row, ["country"]))
            region, continent = country_region_continent(country)
            service_scheme = normalize_service_scheme(first_text(row, ["Service scheme", "service_type", "service scheme"]))
            customer = normalize_customer(first_text(row, ["customer"]))
            ...
            if row_platform_indices:
                ...
                fallback_platform = normalize_platform(first_text(row, ["TurbineType", "platform"]))
                order_data["primary_platform"] = fallback_platform if fallback_platform != "Unknown" else "Unknown"
                if not math.isnan(size_mw):
                    rotor_guess, mw_guess = parse_platform_specs(order_data["primary_platform"])
                    platform_rows.append(
                        {
                            "order_id": order_id,
                            "sheet_year": year,
                            ...
                            "order_size_mw": size_mw,
                            "slot": 0,
                            "platform": order_data["primary_platform"],
                            "turbines_qty": first_float(row, ["NumberofTurbines"]),
                            "rotor_m": rotor_guess,
                            "mw_rating": mw_guess,
                            "slot_mw": size_mw,
                        }
...
def find_default_workbook() -> Path | None:
    preferred = [
        Path("Nordex_data_new.xlsx"),
        Path("Nordex_data.xlsx"),
        Path("nordex_data_new.xlsx"),
    ]
    for p in preferred:
    ...
    nordex_files = sorted([f for f in Path(".").glob("*Nordex*.xlsx") if not f.name.startswith("~$")])
    if nordex_files:
        return nordex_files[0]
...
def is_nordex_relevant(
    ...
    if any(bad in full for bad in NORDEX_NEWS_BLACKLIST):
        return False

    # Official Nordex channels are already company-specific.
    if source_txt.startswith("nordex (official"):
        return True

    title_txt = title or ""
    if not NORDEX_TITLE_RE.search(title_txt):
        return False

    # Filter broad roundup headlines where Nordex is one of many names.
    title_low = title_txt.lower()
    if title_low.count("|") >= 2 and not title_low.startswith("nordex"):
        return False

    return any(k in full for k in NORDEX_KEYWORDS)


def nordex_news_focus(news_type: Any) -> str:
...
def parse_nordex_company_news_archive() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    url = "https://www.nordex.com/en/media/company-news/_jcr_content/root/container/container/newsarchive.newsarchive.json"
    try:
    ...
    for item in news_items:
        ...
        if title is None or path is None:
            continue
        link = path if path.startswith("http") else f"https://www.nordex.com{path}"
        ...
        if key in seen:
            continue
        if not is_nordex_relevant(title, summary, link, source_name="Nordex (Official News)"):
            continue
        seen.add(key)
        rows.append(
            {
                "source": "Nordex (Official News)",
                "focus": nordex_news_focus(item.get("type")),
                "title": title,
                "link": link,
                "summary": summary,
                "published": published,
    ...
    for node in items:
        ...
        if source_name.startswith("Google News") and " - " in title:
            ...
            if t_src is not None:
                source = t_src
        if not is_nordex_relevant(title, summary, link, source_name=source):
            continue
        rows.append(
            {
                "source": source,
...
def parse_nordex_orders_page() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    url = "https://www.nordex.com/en/investor/announcements/wind-turbines-orders"
    try:
        resp = requests.get(url, timeout=20, headers={"User-Agent": "Mozilla/5.0"})
        resp.raise_for_status()
        page = resp.text
    ...
    for d_str, link, title in pattern.findall(page):
        ...
        if title_n is None or link_n is None:
            continue
        if not is_nordex_relevant(title_n, None, link_n, source_name="Nordex (Official Orders)"):
            continue
        rows.append(
            {
                "source": "Nordex (Official Orders)",
                "focus": "company",
                "title": title_n,
                "link": link_n,
                "summary": None,
...
def load_latest_news() -> pd.DataFrame:
    rows: list[dict[str, Any]] = parse_nordex_company_news_archive()
    rows.extend(parse_nordex_orders_page())
...
def load_latest_patents() -> pd.DataFrame:
    ...
    for row_html in re.findall(r"<tr data-ri=\"\d+\".*?</tr>", page, flags=re.IGNORECASE | re.DOTALL):
        ...
        if title is None or publication_number is None:
            continue
        if applicant is not None and "nordex" not in applicant.lower():
            continue
...
def render_information_page() -> None:
    ...
    st.write(
        "All figures are compiled manually from Nordex public investor communications (press releases/announcements) and annual reports."
    )
    ...
    st.write(
        "This is an unofficial, independent dashboard and is not affiliated with, endorsed by, or sponsored by Nordex."
    )
...
def render_latest_news() -> None:
    ...
    st.caption("Showing the 20 latest items directly related to Nordex.")
    st.caption("Sources include official Nordex news plus selected financial and wind-industry feeds.")
    ...
    for idx, row in enumerate(news.itertuples(index=False), start=1):
        ...
        if summary is None or len(summary) < 80:
            summary = (
                f"This is a Nordex-related {focus} update from {source}. "
...
def render_latest_patents() -> None:
    ...
    st.caption("Showing 10 most recent Nordex patent publications from WIPO PATENTSCOPE.")
    st.caption("Inventor names are listed from filings; employment is inferred from Nordex applicant records.")
...
def render_overall_economics(economy: pd.DataFrame, stock_monthly: pd.DataFrame, market_monthly: pd.DataFrame) -> None:
    ...
    if economy.empty:
        st.info("No parsable data found in `Nordex Economy`.")
        return
    ...
    revenue_metric = find_metric(["revenue"]) or find_metric(["gross", "revenue"])
    gross_profit_metric = find_metric(["gross", "profit"]) or find_metric(["ebit"])
    ...
    deliveries_mw_metric = find_metric(["deliveries", "mw"]) or find_metric(["installed", "capacity", "mw"])
    ...
    with g1:
        plot_metric_group(
            ...
            "Economics Metrics (Revenue, EBIT/Sales, Staff Cost)",
            "Value",
        )
    with g2:
        plot_metric_group(
            [order_intake_beur_metric, order_backlog_wt_beur_metric, order_backlog_service_beur_metric],
            "Order Intake / Backlog Metrics (Currency)",
            "Value",
        )

    plot_metric_group(
        [order_intake_mw_metric, order_backlog_mw_metric, deliveries_mw_metric],
        "MW Metrics (Order Intake / Capacity / Delivery)",
        "MW",
    )

    st.markdown("**Nordex Stock Price (Monthly OHLC)**")
    if stock_monthly is None or stock_monthly.empty:
        st.info("No local stock file found. Expected: `data/nordex_stock_monthly.json`.")
    else:
        ...
        if stock.empty:
            ...
            fig_stock.update_layout(
                template=plotly_template(),
                title="Nordex (NDX1.DE) Monthly Price - Open/High/Low/Close",
                yaxis_title="Price",
                xaxis_title="Date",
            ...
            st.caption("Source: local file `data/nordex_stock_2008_2026.txt` (converted to monthly OHLC JSON), plus HRC=F (steel) and HG=F (copper).")
...
def main() -> None:
    ...
    if workbook is None:
        ...
        if cached is None:
            st.error("No Excel workbook found and no parsed cache available (`data_cache/nordex_parsed_data.pkl`).")
            return
